...
export const meta: Route.MetaFunction = () => {
  return [{ title: "React Bridging Assets and Branding Guidelines" }];
};

const BRAND_DIR = "/_brand/React Bridging Brand Assets";
...
export default function Brand() {
  return (
    <div className="prose container my-8 flex max-w-full flex-col gap-8 text-base sm:text-lg lg:my-24 lg:max-w-4xl">
      ...
      <p>The React Bridging name and logos are trademarks of Shopify Inc.</p>
      <p>
        You may not use the React Bridging name or logos in any way that could
        mistakenly imply any official connection with or endorsement of Shopify
        Inc. Any use of the React Bridging name or logos in a manner that could
      ...
      <p>
        You can download a zip file containing all the React Bridging brand
        assets:
      </p>
      <p>
        <a
          href="/_brand/React Bridging Brand Assets.zip"
          className="underline opacity-50 hover:opacity-100"
          download
        >
          React Bridging Brand Assets
        </a>
      </p>
...
function Logos({
  ...
  let filePath = `${BRAND_DIR}/React Bridging ${title}`;
  if (subTitle) {
    filePath += `/${subTitle}`;
  }
  if (oneColor) {
