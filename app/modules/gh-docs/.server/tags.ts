...
global.tagsCache ??= new LRUCache<string, string[]>({
  ...
  fetchMethod: async (key) => {
    ...
    return getAllReleases(owner, repo, "react-bridging");
  },
});

// TODO: implementation details of the react bridging site leaked into here cause
    ...
    ...data
      .filter((release) => {
        return Boolean(
          ...
          // release notes (sometimes react-bridging, sometimes react-dom) so we
          ...
          release.name?.startsWith("v6") ||
            ...
            // After changesets, we look for react-bridging@6.4.0
      ...
      .map((release) => {
        return (
          ...
          release.tag_name.startsWith("v6")
            ? release.tag_name
            : // with changesets its like react-bridging@6.4.0
